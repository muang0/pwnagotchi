...
build {
  ...
  provisioner "shell" {
    inline = ["export LC_ALL=C && export LANG=en_US.UTF-8"]
  }
  provisioner "shell" {
    inline = ["export LC_ALL=C && export LANG=en_US.UTF-8 && apt-get clean && apt-get update && apt-get install -y locales-all"]
  }
  provisioner "shell" {
    inline = ["/usr/sbin/update-locale LANG=en_US.UTF-8 LANGUAGE=en_US.UTF-8 LC_ALL=en_US.UTF-8"]
  }
  provisioner "shell" {
    inline = ["/usr/sbin/locale-gen en_US.UTF-8"]
  }
  provisioner "shell" {
